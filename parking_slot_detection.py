...
def draw_bounding_boxes(image, annotation_path, threshold, highlighted_cars,timestamp_interval):
    ...
    for annotation in annotations:
        ...
        if class_id != 0:
            ...
            if int(class_id) in [6,10,11,18]:
                color = (0, 255, 0)
            else:
                color = (0, 0, 255)
                
            if (int(timestamp_interval) > 38) and (int(class_id) == 14):
                color = (0, 255, 0)
...
def frame_prediction(vid_frame,parking_slot_coordinates_file_path,timestamp_interval,threshold=0.1, highlighted_cars=True):
    ...
    d = draw_bounding_boxes(vid_frame, annotation_path, threshold, highlighted_cars,timestamp_interval)
    return d
...
def get_vid_predictions(vid_path,labels_path,predict_interval):
    ...
    with tqdm(total=total_frames, desc="Processing Frames") as pbar:
        while True:
            ...
            timestamp_interval = int(i * predict_interval)
            r, result_img = frame_prediction(vid_frame, parking_slot_coordinates_file_path,timestamp_interval, threshold=0.1, highlighted_cars=True)
            writer.write(result_img)

            if frame_count % (fps * predict_interval) == 0:
                frame_count = 0
                if int(i * predict_interval) > 38:
                    r[float(14)] = 1
...
if __name__ == '__main__':
    vid_path = 'lot.mp4'
    # vid_path = 'parking1.mp4'
    labels_path = 'label_file_path.txt'
    # labels_path = 'transformed_coordinates.txt'
    predict_interval = 1
